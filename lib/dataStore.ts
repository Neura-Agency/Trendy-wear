...
class DataStore extends EventEmitter {
  ...
  addOrder(order: any): void {
    ...
    if (si) {
      ...
      // Keep master inventory as TOTAL stock (alloted + unalloted): reduce total on any sale
      const invTotal = this.data.inventory.find(i => i.productName === order.productName);
      if (invTotal) {
        invTotal.quantityAvailable = Math.max(0, invTotal.quantityAvailable - (order.quantity || 1));
      }
    } else {
    ...
    if (order.storeName && order.storeName !== 'Direct' && !this.data.stores[order.storeName]) {
      this.data.stores[order.storeName] = { 
        commission: this.data.settings.defaultCommission, 
        paidAmount: 0, 
        paid: false, 
  ...
  updateOrderCommission(orderId: string, commissionPercent: number): any {
    this._load();
    ...
    const gross = (order.sellingPrice || 0) * (order.quantity || 1);
    const cost = (order.costPrice || 0) * (order.quantity || 1);
    const shipment = order.shipmentCost || 0;
    const netAfterShipment = gross - shipment;
  ...
  updateInventoryItem(productName, batchNumber, fields) {
    ...
    if (!item) return null;
    if (fields && Object.prototype.hasOwnProperty.call(fields, 'quantityAvailable')) {
      const next = typeof fields.quantityAvailable === 'string' ? parseInt(fields.quantityAvailable, 10) : fields.quantityAvailable;
      fields.quantityAvailable = Math.max(0, Number.isFinite(next) ? next : (item.quantityAvailable || 0));
    }
  ...
  adjustInventoryQuantity(productName, batchNumber, quantityDelta) {
    this._load();
    const item = this.data.inventory.find(i => i.productName === productName && i.batchNumber === batchNumber);
    if (!item) return null;
    const delta = typeof quantityDelta === 'string' ? parseInt(quantityDelta, 10) : quantityDelta;
    const safeDelta = Number.isFinite(delta) ? delta : 0;
    item.quantityAvailable = Math.max(0, (item.quantityAvailable || 0) + safeDelta);
    this._save();
    return item;
  }
  ...
  updateStoreCommission(storeName, commission) {
    ...
    this.data.stores[storeName].commission = typeof commission === 'string' ? parseFloat(commission) : commission;
    this._save();
  ...
  // ── STORE INVENTORY ──────────────────────────────────────
  assignItemToStore(storeName, productName, ownerSupplyPrice, quantity, commissionPercent, owner) {
    this._load();
    quantity = parseInt(quantity) || 0;
    ownerSupplyPrice = parseFloat(ownerSupplyPrice) || 0;

    // Warehouse inventory quantityAvailable is treated as TOTAL stock.
    // Only allow allotment up to (totalQty - alreadyAlloted).
    const inv = this.data.inventory.find(i => i.productName === productName);
    const totalQty = Number(inv?.quantityAvailable) || 0;
    let alreadyAlloted = 0;
    Object.values(this.data.storeInventory || {}).forEach((items: any) => {
      const it = items?.[productName];
      if (it) alreadyAlloted += (Number(it.quantityAssigned) || 0);
    });
    const availableToAllot = Math.max(0, totalQty - alreadyAlloted);
    if (quantity > availableToAllot) {
      return { error: `Quantity cannot be more than ${availableToAllot}` };
    }
  ...
  addClientPayment(clientId, amount) {
    ...
    const client = this.data.clients.find(c => c.id === clientId);
    if (!client) return null;
    client.paymentsReceived = (client.paymentsReceived || 0) + (typeof amount === 'string' ? parseFloat(amount) : amount);
    this._save();
    return client;
  }
}
